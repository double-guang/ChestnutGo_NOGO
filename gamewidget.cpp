...
void GameWidget::showAboutWidget()
{
    ...
    AboutBox.setText("Name: 栗宝向前冲<br><br>"
                     "Member: 关玉卓 陈庆喜 于歆照<br><br>"
                     "Powered by QT 6.2.12");
    AboutBox.exec();
}
...
void GameWidget::displayHint(int number)
{
    ...
    case 2: //位置无效
        ...
        if(referee->m_color == BLACK) //红方无效
            h->setStyleSheet("background-color:rgba(166,123,142,200);border-radius:20px;"
                             "font: 24px 'Arial Rounded MT Bold'");
        else if(referee->m_color == WHITE) //蓝方无效
            h->setStyleSheet("background-color:rgba(255,219,233,200);border-radius:20px;"
                             "font: 24px 'Arial Rounded MT Bold'");
        break;
    case 3: //红方胜
        h = new HintWidget("WINNER: BLACK", this);
        h->setStyleSheet("background-color:rgba(166,123,142,200);border-radius:20px;"
                         "font: 24px 'Arial Rounded MT Bold'");
        break;
    case 4: //蓝方胜
        h = new HintWidget("WINNER: WIHTE", this);
        h->setStyleSheet("background-color:rgba(255,229,233,200);border-radius:20px;"
                         "font: 24px 'Arial Rounded MT Bold'");
        break;
    default:
        break;
...
void GameWidget::paintEvent(QPaintEvent *event)
{
    ...
    */
    ...
    {
        pen.setWidth(4);
        pen.setColor(QColor("#FFE6F3"));
        painter.setPen(pen);
        brush.setColor(QColor("#FFE6F3"));
    ...
    for (int i = 0; i < kBoardSizeNum; i++)
        for (int j = 0; j < kBoardSizeNum; j++)
        {
            ...
            {
                pen.setColor(QColor("#FFDBE9"));
                brush.setColor(QColor("#3C2B32"));
                painter.setBrush(brush);
                painter.setPen(pen);
